...
(function () {
  ...
  function visual(v) {
    if (v.kind === 'ruler') {
      const x = n => 25 + n * 38;
      return `<figure class="learning-visual"><svg viewBox="0 0 440 125" role="img" aria-label="${esc(`Ruler marked in ${v.unit}. Pencil begins at ${v.start || 0} and ends at ${v.end}.`)}"><rect x="15" y="65" width="410" height="55" rx="8" fill="#ffe299"/>${Array.from({length:11},(_,n)=>`<path d="M${x(n)} 65v15" stroke="#243548"/><text x="${x(n)}" y="100" text-anchor="middle" fill="#243548" font-size="14">${n}</text>`).join('')}<text x="415" y="115" text-anchor="end" fill="#243548" font-size="12">${esc(v.unit)}</text><path d="M${x(v.start || 0)} 27H${x(v.end)-16}l16 12-16 12H${x(v.start || 0)}Z" fill="#388ee3" stroke="#243548"/><path d="M${x(v.end)-16} 27l16 12-16 12Z" fill="#e4b38e"/><path d="M${x(v.end)-5} 35l5 4-5 4Z" fill="#243548"/></svg><figcaption>Read the marked ruler, not the size on your screen.</figcaption></figure>`;
    }
    ...
    if (v.kind === 'blocks') return `<figure class="learning-visual"><div class="blocks">${['Hundreds','Tens','Ones'].map((label,i)=>`<div><b>${label}</b><div class="block-icons">${Array.from({length:v.values[i]},()=>`<span class="block-${i}" aria-hidden="true">${['▦','▮','▪'][i]}</span>`).join('') || '—'}</div><small>${v.values[i]} ${label.toLowerCase()}</small></div>`).join('')}</div></figure>`;
